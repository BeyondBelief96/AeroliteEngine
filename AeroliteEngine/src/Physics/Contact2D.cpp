...
namespace Aerolite {
    ...
    void Contact2D::ResolveImpulse()
    {
        ...
        // Vectors to store the impulses and contact points for each contact.
        // These are used to apply the impulses at the end.
        std::vector<Aerolite::Vec2> impulses;
        std::vector<Aerolite::Vec2> contactPoints;

        // Reserve space for the vectors based on the number of contacts.
        // This can improve performance by reducing memory reallocations.
        impulses.reserve(contactCount);
        contactPoints.reserve(2 * contactCount);

        // Iterate over each contact point. The loop runs for the number of contacts in the collision.
        for (int i = 0; i < contactCount; ++i)
        {
            // Determine the current contact point based on the loop index.
            const Vec2 contact = (i == 0) ? contact1 : contact2;
            ...
            // Store the calculated impulse and the contact points.
            // Impulse is a vector quantity; it has both magnitude and direction.
            // The direction is along the collision normal.
            impulses.emplace_back(j);
            contactPoints.emplace_back(ra);
            contactPoints.emplace_back(rb);
        }

        // Apply the calculated impulses to each object.
        // For each impulse, apply it to object A and the opposite impulse to object B.
        // This ensures that the total linear and angular momentum of the system is conserved.
        for (size_t i = 0; i < impulses.size(); ++i)
        {
            const auto& j = impulses[i];
            const auto& ra = contactPoints[2 * i];
            const auto& rb = contactPoints[2 * i + 1];
